...
class Warehouse:
    ...
    def __init__(self, root):
        ...
        self.root.configure(bg="#FFDAB9")  # Set light orange background
    ...
    def create_widgets(self):
        heading_label = tk.Label(self.root, text="Warehouse Management", font='Helvetica 16 bold', bg="#FFDAB9",fg="#000000")
        ...
        ttk.Button(self.root, text="REMOVE ORDER BY ID", command=self.remove_order_by_id).grid(row=11, column=4, pady=10)
        ...
        ttk.Button(self.root, text="LOAD ORDERS", command=self.load_orders).grid(row=11, column=6, columnspan=2, pady=10)
        ttk.Button(self.root, text="SIMULATE WORKFLOW", command=self.simulate_work_flow).grid(row=12, column=6, columnspan=2, pady=10)
    ...
    def adding_product(self):
        ...
        # Update the product information text area
    ...
    # Check if the maximum order limit is reached
        ...
        new_order.logging_order_information()
    ...
    def removing_order(self):
        ...
        for order in orders_to_remove:
            if order.status == "collected":
                ...
                # Print orders before removal
                print("Orders before removal:", self.orders)

                # Remove the order from the list
                self.orders.remove(order)

                # Print orders after removal
                print("Orders after removal:", self.orders)
                ...
                messagebox.showinfo("Success", f"Order {order.order_id} removed successfully.")
    ...
    def mark_collected_order_id(self):
        order_id = simpledialog.askstring("Mark Collected Order", "Enter Order ID to put in collection queue:")
    ...
    def load_order_json(self, filename):
        try:
            with open(filename, "r") as file:
                order_datafile = json.load(file)
            for order_datafile in order_datafile:
                new_order = Order()
                for product_info in order_datafile.get("products",[]):
                    product = Product(
                        name = product_info.get("name", ""),
                        quantity=int(product_info.get("quantity", 0)),
                        specification= product_info.get("specification",""),
                        last_ordered=product_info.get("last_ordered","")
                    )
                    new_order.add_product(product)
                new_order.set_status("registered")
                self.orders.append(new_order)
            self.update_orders_text_area()
            messagebox.showinfo("Success", f"Order Loaded Sucess from {filename}")
        except Exception as e:
            messagebox.showerror("Error", f"Error While Loading the file from {filename}: {e}")    
                
    def load_orders(self):
        filename = askopenfilename(title="Select JSON File", filetypes=[("JSON files", "*.json")])
        if filename:
            self.load_order_json(filename)

    #Simulate for the single order
    def simulate_single_order_data(self, order):
        if order.status != "collected":
            order.set_status("in Collection Queue")
            order.mark_collected()
            #update collect order
            self.remove_collected_orders()
            self.update_orders_text_area()
            self.order_counter -=1
            order.log_order_update()

    #simulating for thread of workflow
    def simulate_work_thread(self):
        #collcted order for accepted time
        order_simulate = sorted(self.orders,key=lambda x: x.time_accepted)

        #thread for currency
        with concurrent.futures.ThreadPoolExecutor()as executor:
            #submitting order for each task and order
            futures = [executor.submit(self.simulate_single_order_data, order) for order in order_simulate]

            #wait for task to complete
            concurrent.futures.wait(futures)

        #schedule the update order text area afrer delay
        self.root.after(500, self.update_orders_text_area)
        messagebox.showinfo("SIMULATION COMPLETE", "WORKFOW OF THE SIMULATION COMPLETED")


    def simulate_work_flow(self):
        simulate_thread = threading.Thread(target= self.simulate_work_thread)
